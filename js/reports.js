document.addEventListener('DOMContentLoaded', function() {
    ...
    function showTab(tabId) {
        ...
        loadReportData(tabId, 'name', 'asc'); // Default sort for new tab
    }
    ...
    if (applyDetailedInventoryFiltersBtn) {
        applyDetailedInventoryFiltersBtn.addEventListener('click', function() {
            ...
            loadReportData('detailed-inventory', 'name', 'asc'); // Default sort on filter apply
        });
    }

    // Function to load report data via AJAX
    function loadReportData(reportType, sortBy = 'name', sortOrder = 'asc') {
    ...
    function renderReport(reportType, data, userRole, totalRecords) {
        ...
        switch (reportType) {
            ...
            case 'detailed-inventory':
                if (data.length === 0) {
                    ...
                    html = `
                        <table class="sortable-table" data-report-type="${reportType}">
                            <thead>
                                <tr>
                                    <th data-sort-by="name">Name <span class="sort-icon"></span></th>
                                    <th data-sort-by="category_name">Category <span class="sort-icon"></span></th>
                                    <th data-sort-by="location_name">Location <span class="sort-icon"></span></th>
                                    <th data-sort-by="current_stock">Stock <span class="sort-icon"></span></th>
                                    <th>Unit</th>
                                    <th>Low Stock Threshold</th>
                                    <th>Max Stock</th>
                                    <th>Perishable</th>
                                    <th data-sort-by="expiry_date">Expiry Date <span class="sort-icon"></span></th>
                ...
                if (data.length === 0) {
                    ...
                    html = `
                        <table class="sortable-table" data-report-type="${reportType}">
                            <thead>
                                <tr>
                                    <th data-sort-by="log_id">Log ID <span class="sort-icon"></span></th>
                                    <th data-sort-by="item_name">Item Name <span class="sort-icon"></span></th>
                                    <th data-sort-by="action">Action <span class="sort-icon"></span></th>
                                    <th data-sort-by="date_time">Date/Time <span class="sort-icon"></span></th>
                                </tr>
                            </thead>
                            <tbody>
                    `;
        ...
        if (totalRecords !== undefined) {
            renderPagination(reportType, totalRecords);
        }

        // Add sort event listeners to table headers
        addSortEventListeners(reportType);
    }

    // Function to render pagination controls
    function renderPagination(reportType, totalRecords) {
        const totalPages = Math.ceil(totalRecords / itemsPerPage);
        let paginationHtml = '<div class="pagination">';

        if (totalPages > 1) {
            paginationHtml += `<button class="pagination-button" data-page="1" ${currentPage === 1 ? 'disabled' : ''}>&laquo; First</button>`;
            paginationHtml += `<button class="pagination-button" data-page="${currentPage - 1}" ${currentPage === 1 ? 'disabled' : ''}>&lsaquo; Previous</button>`;
            ...
            paginationHtml += `<button class="pagination-button" data-page="${currentPage + 1}" ${currentPage === totalPages ? 'disabled' : ''}>Next &rsaquo;</button>`;
    ...
    function addSortEventListeners(reportType) {
        const table = document.querySelector(`#${reportType}-data .sortable-table`);
        if (table) {
            const headers = table.querySelectorAll('th[data-sort-by]');
            headers.forEach(header => {
                header.addEventListener('click', function() {
                    const sortBy = this.dataset.sortBy;
                    let sortOrder = this.dataset.sortOrder || 'asc'; // Default to asc
                    ...
                    // Update sort icon for current header
                    this.querySelector('.sort-icon').innerHTML = sortOrder === 'asc' ? ' &#9650;' : ' &#9660;'; // Up or Down arrow

                    // Reload data with sorting
                    loadReportData(reportType, sortBy, sortOrder);
                });
            });
        }
    }
