...
function addOrUpdateItemRow(item) {
    ...
    if (existingRow) {
        ...
        let status = 'Normal';
        let status_class = 'btn btn--success'; // Default to green

        if (item.quantity <= item.low_stock_threshold || item.quantity <= item.min_stock_level) {
            status = 'Low Stock';
            status_class = 'btn btn--warning'; // Yellow for low stock
        }
        if (item.quantity == 0) {
            status = 'Out of Stock';
            status_class = 'btn btn--danger'; // Red for out of stock
        }
        if (item.max_stock_level > 0 && item.quantity >= item.max_stock_level) {
            status = 'Surplus';
            status_class = 'btn btn--info'; // Blue for surplus (optional, can be green)
        }
        
        statusCell.innerHTML = `<span class="${status_class}">${status}</span>`;

        // Update row class for styling
        existingRow.classList.remove('alert--warning', 'alert--error', 'alert--info'); // Remove old classes
        if (status === 'Low Stock') existingRow.classList.add('alert--warning');
        if (status === 'Out of Stock') existingRow.classList.add('alert--danger');
        if (status === 'Surplus') existingRow.classList.add('alert--info'); // Optional: add a class for surplus
        ...
        if (status === 'Low Stock') newRow.classList.add('alert', 'alert--warning');
        if (status === 'Out of Stock') newRow.classList.add('alert', 'alert--danger');
        if (status === 'Surplus') newRow.classList.add('alert', 'alert--info'); // Optional: add a class for surplus
