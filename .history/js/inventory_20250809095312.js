...
let last_poll_timestamp = ''; // Stores the timestamp of the last successful poll

// Function to fetch and update inventory items
function pollForUpdates() {
    // Get the current timestamp to send with the request
    const currentTimestamp = new Date().toISOString().slice(0, 19).replace('T', ' '); // Format: YYYY-MM-DD HH:MM:SS

    fetch(`ajax/get_latest_item_updates.php?last_timestamp=${encodeURIComponent(last_poll_timestamp)}`)
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response was not ok: ' + response.statusText);
            }
        ...
        .then(data => {
            if (data.success && data.items.length > 0) {
                ...
                // Update the last_poll_timestamp with the server's current time
                // This ensures we only fetch changes since the server's last response
                last_poll_timestamp = data.server_time;
            } else if (!data.success) {
                console.error('[pollForUpdates] Error fetching updates:', data.message);
            }
        })
        .catch(error => {
            console.error('[pollForUpdates] Fetch error:', error);
            // Optionally display a non-intrusive error message to the user
            // displayGUIMessage('Failed to get real-time updates. Please refresh the page.', 'error');
        });
}

// Initial call to set the last_poll_timestamp and start polling
document.addEventListener('DOMContentLoaded', function() {
    // Get the initial server time from a hidden element or a direct AJAX call if needed
    // For simplicity, we'll assume the first poll will fetch all items,
    // and subsequent polls will use the server_time from the response.
    // Or, if the page already renders items, we can get the latest updated_at from the existing items.
    // For now, we'll let the first poll fetch everything if last_poll_timestamp is empty.
