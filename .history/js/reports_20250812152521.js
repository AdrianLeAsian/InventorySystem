document.addEventListener('DOMContentLoaded', function() {
    ...
    // Event Listeners for Filter Items Report
    ...
    let usageTrendsCtx = document.getElementById('monthlyUsageChart'); // Changed to let
    ...
    function fetchUsageTrends() {
        ...
        fetch(`ajax/get_usage_trends.php?${params.toString()}`)
            .then(response => response.json())
            .then(data => {
                console.log('Usage Trends Data:', data); // Log the data for debugging
                if (data.success) {
                    ...
                    usageTrendsCtx.insertAdjacentHTML('afterend', `<p class="text-center text-muted usage-trends-message">${data.message || 'No data available for the selected period to display usage trends.'}</p>`);
    ...
    function renderUsageTrendsChart(usageData, chartXAxisLabel, viewType) {
        ...
        usageData.forEach(record => {
            ...
            itemNames.add(record.item_name);

            if (!dataIn[record.item_name]) {
                dataIn[record.item_name] = {};
            }
            if (!dataOut[record.item_name]) {
                dataOut[record.item_name] = {};
            }

            dataIn[record.item_name][record.date_label] = parseInt(record.total_quantity_in);
            dataOut[record.item_name][record.date_label] = parseInt(record.total_quantity_out);
        });
        ...
        const chartDatasets = [];
        const colorsIn = ['rgba(75, 192, 192, 0.8)', 'rgba(153, 102, 255, 0.8)', 'rgba(255, 159, 64, 0.8)', 'rgba(54, 162, 235, 0.8)', 'rgba(255, 99, 132, 0.8)', 'rgba(201, 203, 207, 0.8)'];
        const colorsOut = ['rgba(255, 99, 132, 0.8)', 'rgba(54, 162, 235, 0.8)', 'rgba(255, 206, 86, 0.8)', 'rgba(75, 192, 192, 0.8)', 'rgba(153, 102, 255, 0.8)', 'rgba(255, 159, 64, 0.8)'];
        let colorIndex = 0;
        ...
        itemNames.forEach(itemName => {
            ...
            chartLabels.forEach(label => {
                dataPointsIn.push(dataIn[itemName][label] || 0);
                dataPointsOut.push(dataOut[itemName][label] || 0);
            });

            chartDatasets.push({
                label: `${itemName} (In)`,
                data: dataPointsIn,
                borderColor: colorsIn[colorIndex % colorsIn.length],
                backgroundColor: colorsIn[colorIndex % colorsIn.length],
                fill: false,
                tension: 0.1
            });
            ...
            chartDatasets.push({
                label: `${itemName} (Out)`,
                data: dataPointsOut,
                borderColor: colorsOut[colorIndex % colorsOut.length],
                backgroundColor: colorsOut[colorIndex % colorsOut.length],
                fill: false,
                tension: 0.1,
                borderDash: [5, 5] // Dotted line for 'Out'
            });
            colorIndex++;
        });
