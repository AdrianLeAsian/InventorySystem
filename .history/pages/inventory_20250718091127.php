...
$sql_fetch_items = "SELECT i.id, i.name, i.category_id, i.barcode, i.quantity, i.unit, i.low_stock_threshold, i.min_stock_level, i.max_stock_level, i.description, i.updated_at, c.name as category_name, i.location
                    FROM items i
...
foreach ($all_items as $item) {
    if (($item['low_stock_threshold'] > 0 && $item['quantity'] <= $item['low_stock_threshold']) || ($item['min_stock_level'] > 0 && $item['quantity'] <= $item['min_stock_level'])) {
                                    ...
                                    <tbody>
                                        <?php if (!empty($all_items)): ?>
                                            <?php foreach ($all_items as $item): ?>
                                                <?php
                                                    $status = 'Normal';
                                                    $status_class = 'btn btn--success'; // Green for normal/fully stocked
                                                    ...
                                                    if ($item['max_stock_level'] > 0 && $item['quantity'] >= $item['max_stock_level']) {
                                                        $status = 'Surplus';
                                                        $status_class = 'btn btn--info'; // Blue for surplus
                                                    }
                                                ?>
                                                <tr data-item-id="<?php echo htmlspecialchars($item['id']); ?>" data-category-id="<?php echo htmlspecialchars($item['category_id']); ?>" class="table__row">
                                                    ...
                                                    <button class="btn btn--primary" onclick="openEditItemModal(<?php echo $item['id']; ?>)">Edit</button>
                                                            <a href="index.php?page=delete_item&id=<?php echo $item['id']; ?>" class="btn btn--danger">Delete</a>
                                                        </div>
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
                                        <?php else: ?>
                                            <tr class="table__row">
                                                <td colspan="12" class="table__cell text-center">No items found.</td>
                                            </tr>
                                        <?php endif; ?>
                                    </tbody>
                                </table>
