...
function getTotalRecords($conn, $report_type, $filters = []) {
    $where_clauses = [];
    $params = [];
    $param_types = '';
    $table = '';
    ...
    switch ($report_type) {
        case 'detailed-inventory':
            $table = 'items';
            // Build WHERE clauses based on filters (similar to getDetailedInventory)
            if (!empty($filters['search'])) {
                $where_clauses[] = "items.name LIKE ?";
                $params[] = '%' . $filters['search'] . '%';
                $param_types .= 's';
        ...
        case 'transaction-logs':
            $table = 'logs';
            // No specific filters for logs yet, but can be added here
            break;
        case 'expiry-calendar':
            // This is more complex as it merges two tables.
            // For total count, we need to count unique items based on the merging logic.
            // For simplicity, we'll count distinct item_name + expiry_date combinations.
            $table = 'items'; // Base table for counting perishable items
            $where_clauses[] = "is_perishable = 1 AND expiry_date IS NOT NULL";
            break;
        default:
            return 0;
    }
    ...
    $sql = "SELECT COUNT(*) AS total_records FROM {$table} {$where_sql}";
    
    // Special handling for expiry-calendar count due to merging
    if ($report_type === 'expiry-calendar') {
        $sql = "SELECT COUNT(DISTINCT CONCAT(i.name, ib.expiry_date)) AS total_records
                FROM item_batches ib
                JOIN items i ON ib.item_id = i.id
                WHERE i.is_perishable = 1 AND ib.expiry_date IS NOT NULL AND ib.quantity > 0";
        // This count doesn't include items with expiry_date directly in items table without batches.
        // A more robust solution would involve a UNION ALL subquery for counting.
        // For now, this provides a reasonable count for the primary source of expiry data.
        $stmt = $conn->prepare($sql);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        // Add count from items table directly
        $sql_items_count = "SELECT COUNT(*) AS total_records FROM items WHERE is_perishable = 1 AND expiry_date IS NOT NULL AND current_stock > 0";
        $stmt_items_count = $conn->prepare($sql_items_count);
        $stmt_items_count->execute();
        $result_items_count = $stmt_items_count->get_result();
        $total_records += $result_items_count->fetch_assoc()['total_records'];
        $stmt_items_count->close();
    ...
    $stmt = $conn->prepare($sql);
    if ($stmt === false) {
        error_log("Prepare failed (getTotalRecords): " . $conn->error);
        return 0;
    }
...
function getTransactionLogs($conn, $limit = 10, $offset = 0) {
    $logs = [];
...
function getExpiryCalendar($conn, $limit = 10, $offset = 0) {
    $expiry_items = [];
    $today = date('Y-m-d');
    ...
    $paginated_items = array_slice($expiry_items, $offset, $limit);
