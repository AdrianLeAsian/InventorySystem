...
class InventoryImporter {
    ...
    public function import($filePath, $updateExisting, $fileExtension) {
        ...
        try {
            ...
            $stmt_update_item = $this->conn->prepare("UPDATE items SET name = ?, category_id = ?, quantity = ?, unit = ?, low_stock_threshold = ?, min_stock_level = ?, max_stock_level = ?, description = ?, location = ?, updated_at = NOW() WHERE id = ?");
            $stmt_insert_item = $this->conn->prepare("INSERT INTO items (name, category_id, quantity, unit, low_stock_threshold, min_stock_level, max_stock_level, description, location) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
