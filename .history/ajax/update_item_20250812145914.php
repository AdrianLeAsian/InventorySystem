...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($item_id === false || $item_id <= 0) {
        ...
        $sql = "UPDATE items SET name = ?, category_id = ?, quantity = ?, unit = ?, low_stock_threshold = ?, min_stock_level = ?, max_stock_level = ?, description = ?, location = ?, updated_at = NOW() WHERE id = ?";

        // Prepare the statement to prevent SQL injection
        if ($stmt = mysqli_prepare($conn, $sql)) {
            // Bind parameters to the prepared statement
            mysqli_stmt_bind_param($stmt, "siiissssi", 
                $item_name, 
                $item_category_id, 
                $item_quantity, 
                $item_unit, 
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                if (mysqli_stmt_affected_rows($stmt) > 0) {
                    ...
                    if ($logStmt = mysqli_prepare($conn, $logSql)) {
                        ...
                        error_log("Error logging item update to inventory_log: " . mysqli_error($conn));
                    }
                    ...
                    $sql_fetch_updated = "SELECT i.id, i.name, i.category_id, i.quantity, i.unit, i.low_stock_threshold, i.min_stock_level, i.max_stock_level, i.description, i.location, i.updated_at, c.name as category_name
                                          FROM items i
