...
if ($item_status !== null) {
    if ($item_status === 'low_stock') {
        $where_clauses[] = "i.quantity > 0 AND i.quantity <= i.low_stock_threshold";
...
if ($stmt_count) {
    if (!empty($params)) {
        // Use a temporary array for count query parameters to avoid modifying $params for the main query
        $count_params = $params; 
        mysqli_stmt_bind_param($stmt_count, $param_types, ...$count_params);
    }
...
if ($stmt_items) {
    ...
    mysqli_stmt_bind_param($stmt_items, $param_types_with_pagination, ...$params_with_pagination);
