 ...
 * It performs validation, inserts the item into the database,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "INSERT INTO items (name, category_id, quantity, unit, low_stock_threshold, min_stock_level, max_stock_level, description, location) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

    if ($stmt = mysqli_prepare($conn, $sql)) {
        // Bind parameters to the prepared statement
        mysqli_stmt_bind_param($stmt, "sisiisiss", $itemName, $categoryId, $quantity, $unit, $lowStockThreshold, $minStockLevel, $maxStockLevel, $description, $location);
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            $sql_fetch_new_item = "SELECT i.id, i.name, i.category_id, i.quantity, i.unit, i.low_stock_threshold, i.min_stock_level, i.max_stock_level, i.description, i.updated_at, c.name as category_name, i.location
                                   FROM items i
            ...
            if ($stmt_fetch = mysqli_prepare($conn, $sql_fetch_new_item)) {
                ...
                if ($newItem && isset($newItem['updated_at'])) {
                    $newItem['formatted_updated_at'] = format_last_activity($newItem['updated_at']);
                } else {
