...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // 3. Log the activity in the inventory_log table
        $logSql = "INSERT INTO inventory_log (item_id, type, quantity_change, reason) VALUES (?, ?, ?, ?)";
        if (!($logStmt = mysqli_prepare($conn, $logSql))) {
            throw new Exception("Prepare failed for inventory log: " . mysqli_error($conn));
        }
        $logType = ($stockType === 'stock_in') ? 'in' : 'out';
        mysqli_stmt_bind_param($logStmt, "iiss", $itemId, $logType, $quantityChange, $reason);
        if (!mysqli_stmt_execute($logStmt)) {
            throw new Exception("Execute failed for inventory log: " . mysqli_stmt_error($logStmt));
        }
        mysqli_stmt_close($logStmt);

        // 4. Log the activity in the activity_log table
        ...
        $sql_fetch_updated_item = "SELECT i.id, i.name, i.category_id, i.quantity, i.unit, i.low_stock_threshold, i.min_stock_level, i.max_stock_level, i.description, i.updated_at, c.name as category_name, i.location
                                   FROM items i
        ...
        if ($stmt_fetch_item = mysqli_prepare($conn, $sql_fetch_updated_item)) {
            ...
            if ($updatedItem && isset($updatedItem['updated_at'])) {
                $updatedItem['formatted_updated_at'] = format_last_activity($updatedItem['updated_at']);
            } else {
