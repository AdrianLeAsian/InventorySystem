...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if ($stmt_check = mysqli_prepare($link, $sql_check_logs)) {
        ...
        if ($log_count > 0) {
            header("Location: index.php?page=inventory&error=has_logs&item_id=" . $item_id);
            exit;
        }
    }
    ...
    if ($stmt = mysqli_prepare($link, $sql)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            header("Location: index.php?page=inventory&error=delete_failed");
            exit;
        }
        mysqli_stmt_close($stmt);
    } else {
        header("Location: index.php?page=inventory&error=prepare_failed");
        exit;
    }
} else {
